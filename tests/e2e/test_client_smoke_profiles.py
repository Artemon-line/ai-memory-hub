...
HTTP_EMBEDDING_BASE_URL = f"{OLLAMA_BASE_URL.rstrip('/')}/v1"
...
def _mcp_client(tmp_path: Any) -> TestClient:
    app = create_app(
        config={
            ...
            "embedding_endpoint": {"base_url": HTTP_EMBEDDING_BASE_URL, "api_key": "ollama"},
...
def test_ollama_chat_completion_smoke() -> None:
    _require_ollama()
    request = Request(
        f"{HTTP_EMBEDDING_BASE_URL}/chat/completions",
        data=json.dumps(
            {
                "model": CHAT_MODEL,
                "messages": [
