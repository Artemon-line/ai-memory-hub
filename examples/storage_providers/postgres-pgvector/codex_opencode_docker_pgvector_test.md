...
- The example `Containerfile` installs only the `postgres` and `tokenizer`
  extras needed for Postgres metadata, PGVector vectors, and token budgeting.
- The example `Containerfile` prewarms `cl100k_base` for `tiktoken`.
...
This test uses `examples/storage_providers/postgres-pgvector/Containerfile`. It is intentionally
slim and installs only the extras this example uses:
...
Runtime selection is separate from installed dependencies. This example activates
only `providers.metadata_db: postgres` and `providers.vector_db: pgvector`.

The example `Containerfile` copies `example.config.yaml` to `/app/config.yaml`.
For this test, `examples/storage_providers/postgres-pgvector/compose.yaml` mounts
...
If the hub exits with `psycopg package is required`, the image was not built with
`--extra postgres`. Recheck `examples/storage_providers/postgres-pgvector/Containerfile`.
